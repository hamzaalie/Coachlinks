...
import { RefreshCw, BarChart3, Download, Users, Activity, UserPlus, DollarSign, Mail as VCard, MessageSquare, CalendarDays, Eye, TrendingUp, Plus, ExternalLink, QrCode, Share2, HardDrive, Clock, Sparkles, AlertTriangle } from 'lucide-react';
  ...
  return (
    ...
    >
      <div className="space-y-6">
        {/* Trial Banner */}
        {auth.user?.is_trial && auth.user?.trial_expire_date && (
          (() => {
            const daysLeft = Math.max(0, Math.ceil((new Date(auth.user.trial_expire_date).getTime() - Date.now()) / (1000 * 60 * 60 * 24)));
            ...
            return (
              <div className={`rounded-xl border p-4 flex flex-col sm:flex-row items-start sm:items-center gap-4 ${
                ...
                <div className={`rounded-full p-2 ${isExpiring ? 'bg-amber-100 dark:bg-amber-900' : 'bg-violet-100 dark:bg-violet-900'}`}>
                  {isExpiring
                ...
                <div className="flex-1">
                  <p className={`text-sm font-semibold ${isExpiring ? 'text-amber-900 dark:text-amber-200' : 'text-violet-900 dark:text-violet-200'}`}>
                    {isExpiring
                      ? t('Your trial expires in {{days}} day(s)!', { days: daysLeft })
                      : t('Your access period has {{days}} day(s) remaining', { days: daysLeft })
                    }
                  </p>
                  <p className={`text-xs mt-0.5 ${isExpiring ? 'text-amber-700 dark:text-amber-300' : 'text-violet-700 dark:text-violet-300'}`}>
                    {isExpiring
                      ? t('Upgrade now to keep your coach card live and avoid losing access.')
                      : t('Choose an active plan any time to continue without interruption.')
                    }
                  </p>
                </div>
                <Link href={route('plans.index')}
                  className={`inline-flex items-center gap-1.5 px-4 py-2 rounded-lg text-sm font-bold text-white transition-all hover:scale-105 ${
                    isExpiring ? 'bg-amber-600 hover:bg-amber-700 shadow-amber-600/20' : 'bg-violet-600 hover:bg-violet-700 shadow-violet-600/20'
                  } shadow-lg`}>
                  {t('Choose a Plan')}
                </Link>
              </div>
          ...
          <Card>
            <CardContent className="p-6">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-sm font-medium text-muted-foreground">{t('Total Coach Cards')}</p>
