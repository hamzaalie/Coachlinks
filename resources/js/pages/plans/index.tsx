...
export default function Plans({ plans: initialPlans, billingCycle: initialBillingCycle = 'monthly', hasDefaultPlan, isAdmin = false, currentPlan, userTrialUsed, paymentMethods = [], onboarding = false, preselectedPlanId = null }: Props) {
  ...
  useEffect(() => {
    if (isAdmin || !onboarding || !preselectedPlanId || isSubscriptionModalOpen) {
      return;
    }

    handleSubscribe(preselectedPlanId);
  }, [isAdmin, onboarding, preselectedPlanId, isSubscriptionModalOpen]);
  ...
  const getActionButton = (plan: Plan) => {
    ...
    // Trial CTA is intentionally disabled for now.
    // if (plan.is_trial_available && !userTrialUsed) { ... }

    return (
      <div className="space-y-2">
        {!onboarding && plan.has_pending_request ? (
          <Button
            ...
            >
              ...
              <div className="relative z-10 flex flex-col h-full p-6 pt-8">
                ...
                <div className="mb-6">
                  ...
                  {/* Trial badge intentionally hidden for now. */}
                </div>
