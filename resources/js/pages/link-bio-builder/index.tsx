...
export default function BioLinkIndex({ bioLinks = { data: [] }, planLimits, filters = {} }: Props) {
  ...
  */
  ...
  const pageActions = [
    {
      label: planLimits && !canCreate ? t('Coach Link Limit Reached ({{current}}/{{max}})', { current: planLimits.current_biolinks, max: planLimits.max_biolinks }) : t('Create New Coach Link'),
      icon: <Plus className="h-4 w-4 mr-2" />,
      variant: canCreate ? 'default' : 'outline',
      onClick: canCreate ? () => router.get(route('link-bio-builder.create')) : () => toast.error(t('Coach link limit exceeded. Your plan allows maximum {{max}} coach links. Please upgrade your plan.', { max: planLimits?.max_biolinks })),
      disabled: !canCreate
    }
  ];
      ...
      {!bioLinks || !bioLinks.data || bioLinks.data.length === 0 ? (
        <div className="bg-white rounded-lg shadow p-8 text-center">
          <p className="text-gray-500 dark:text-gray-400 mb-4">
            {t("You haven't created any coach links yet.")}
          </p>
          <Link href={route('link-bio-builder.create')}>
            <Button>
              <Plus className="w-4 h-4 mr-2" />
              {t("Create Your First Coach Link")}
            </Button>
          </Link>
        </div>
      ) : activeView === 'list' ? (
