...
// import GoogleWalletSettings from './components/google-wallet-settings'; // HIDDEN
...
export default function Settings() {
  ...
  const companyAllowedSections = new Set(['google-calendar-settings', 'webhook-settings']);
  const [activeSection, setActiveSection] = useState(isCompany ? 'google-calendar-settings' : 'system-settings');
  ...
  const sidebarNavItems = allSidebarNavItems.filter(item => {
    if (isCompany) {
      return companyAllowedSections.has(item.href.replace('#', ''));
    }
  ...
  // const googleWalletSettingsRef = useRef<HTMLDivElement>(null); // HIDDEN
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = [
        ...
        // { id: 'google-wallet-settings', ref: googleWalletSettingsRef }, // HIDDEN
    ...
    >
      <div className={`flex flex-col md:flex-row gap-8 ${position === 'right' ? 'md:flex-row-reverse' : ''}`}>
        ...
        <div className="flex-1">
          ...
          {!isCompany && (auth.permissions?.includes('manage-system-settings') || isSuperadmin) && (
          ...
          {!isCompany && (auth.permissions?.includes('manage-currency-settings') || isSuperadmin) && (
          ...
          {!isCompany && (auth.permissions?.includes('manage-email-settings') || isSuperadmin) && (
          ...
          {!isCompany && (auth.permissions?.includes('manage-payment-settings') || isSuperadmin) && (
          ...
          {!isCompany && (auth.permissions?.includes('manage-storage-settings') || isSuperadmin) && (
          ...
          {!isCompany && (auth.permissions?.includes('manage-recaptcha-settings') || isSuperadmin) && (
          ...
          {!isCompany && (auth.permissions?.includes('manage-chatgpt-settings') || isSuperadmin) && (
          ...
          {!isCompany && (auth.permissions?.includes('manage-cookie-settings') || isSuperadmin) && (
          ...
          {!isCompany && (auth.permissions?.includes('manage-seo-settings') || isSuperadmin) && (
          ...
          {!isCompany && (auth.permissions?.includes('manage-cache-settings') || isSuperadmin) && (
          ...
          {(isCompany || auth.permissions?.includes('settings')) && (
          ...
          {/* Google Wallet Settings Section - HIDDEN */}
          ...
          {(auth.permissions?.includes('manage-webhook-settings') && !isSuperadmin) && (
