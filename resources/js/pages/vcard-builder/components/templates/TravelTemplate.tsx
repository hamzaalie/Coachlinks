  ...
  const renderDestinationsSection = (destinationsData: any) => {
    ...
    return (
      ...
      >
        ...
        <div 
          className="flex overflow-x-auto hide-scrollbar mb-4 pb-2" 
          style={{ scrollbarWidth: 'none' }}
        >
          {destinations.map((destination: any, index: number) => (
            <button
              key={index}
              className={`px-4 py-2 mx-1 rounded-full whitespace-nowrap text-sm font-medium flex-shrink-0 transition-all duration-200`}
              style={{ 
