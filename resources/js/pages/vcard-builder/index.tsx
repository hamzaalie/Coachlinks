...
const handleCopyLink = (business: Business) => {
  ...
  const pageActions = [
    {
      label: planLimits && !canCreate ? t('Coach Card Limit Reached ({{current}}/{{max}})', { current: planLimits.current_businesses, max: planLimits.max_businesses }) : t('Create New Coach Card'),
      icon: <Plus className="h-4 w-4 mr-2" />,
      variant: canCreate ? 'default' : 'outline',
      onClick: canCreate ? () => router.get(route('vcard-builder.templates')) : () => toast.error(t('Coach card limit exceeded. Your plan allows maximum {{max}} coach cards. Please upgrade your plan.', { max: planLimits.max_businesses })),
      disabled: !canCreate
    }
  ];
