...
export default function CustomPage() {
  ...
  // Detect if this is a legal page (Privacy Policy, Terms, Refund, etc.)
  const isLegalPage = ['privacy', 'terms', 'refund', 'disclaimer', 'cookie'].some(keyword => 
    page.slug.toLowerCase().includes(keyword) || page.title.toLowerCase().includes(keyword)
  );
  
  // Default subtitle for legal pages
  const getPageSubtitle = () => {
    const title = page.title.toLowerCase();
    if (title.includes('privacy')) return 'Your privacy is important to us. This policy explains how we collect, use, and protect your information.';
    if (title.includes('terms')) return 'Please read these terms and conditions carefully before using our service.';
    if (title.includes('refund')) return 'Our refund and cancellation policy is designed to be fair and transparent.';
    return undefined;
  };
  ...
  return (
    <>
      ...
      >
        {isLegalPage ? (
          <LegalPageHeader settings={settings} />
        ) : (
          <Header 
            settings={settings} 
        ...
        <main className={isLegalPage ? 'pt-24' : 'pt-16'}>
          <div className={isLegalPage ? 'max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12' : ''}>
            <div className="custom-page-content prose prose-lg max-w-none" dangerouslySetInnerHTML={{ __html: page.content }} />
          </div>
        </main>
