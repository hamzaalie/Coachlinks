...
export default function VCardSectionManager({
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(
    () => new Set(sections.map((s: any) => s.key))
  );
          ...
          <div className="space-y-2">
            <Input
              ...
              onChange={(e) => {
                ...
                // Store as array for consistency with template data
                const arr = raw.split(',').map((s: string) => s.trim()).filter(Boolean);
                onUpdateSection(sectionKey, field.name, arr);
              }}
            />
            {Array.isArray(value) && value.length > 0 && (
              <div className="flex flex-wrap gap-1">
                {value.map((tag: string, i: number) => (
                  <span key={i} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs bg-primary/10 text-primary border border-primary/20">
                    {tag}
                    <button
                      type="button"
                      className="hover:text-red-500 text-xs leading-none"
                      onClick={() => {
                        const newTags = value.filter((_: string, idx: number) => idx !== i);
                        onUpdateSection(sectionKey, field.name, newTags);
                      }}
                    >
                      &times;
                    </button>
