...
class PlanController extends Controller
{
    ...
    private function companyPlansView(Request $request)
    {
        ...
        if (session('onboarding_in_progress') && session('onboarding_template') && $user->hasActiveSubscription()) {
            $selectedTemplate = session('onboarding_template');
            session()->forget(['onboarding_in_progress', 'onboarding_plan_id', 'onboarding_template']);

            return redirect()->route('vcard-builder.create', ['template' => $selectedTemplate]);
        }
        ...
        $plans = $dbPlans->map(function ($plan) use ($billingCycle, $user) {
            ...
            return [
                ...
                // Trial flow is intentionally disabled for now.
        ...
        return Inertia::render('plans/index', [
            ...
            'onboarding' => (bool) session('onboarding_in_progress') || $request->boolean('onboarding'),
            'preselectedPlanId' => $request->filled('plan_id')
                ? (int) $request->input('plan_id')
                : (session('onboarding_plan_id') ? (int) session('onboarding_plan_id') : null),
        ]);
    }
    ...
    {
        // Trial flow is intentionally disabled for now.
        // Keep previous implementation for future use.
        // $request->validate([
        //     'plan_id' => 'required|exists:plans,id'
        // ]);
        //
        ...
        return back()->withErrors(['error' => __('Trial is currently disabled.')]);
    }
