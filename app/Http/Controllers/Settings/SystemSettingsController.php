    ...
    public function updateBrand(Request $request)
    {
        try {
            if (!auth()->check() || auth()->user()->type !== 'superadmin') {
                return redirect()->back()->with('error', __('Only superadmin can update brand settings.'));
            }

            if ($request->has('settings.layoutDirection')) {
                $layoutDirection = $request->input('settings.layoutDirection');

                if ($layoutDirection === 'ltr') {
                    $request->merge(['settings' => array_merge($request->input('settings', []), ['layoutDirection' => 'left'])]);
                } elseif ($layoutDirection === 'rtl') {
                    $request->merge(['settings' => array_merge($request->input('settings', []), ['layoutDirection' => 'right'])]);
                }
            }
