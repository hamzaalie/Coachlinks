...
class VCardBuilderController extends Controller
{
    ...
    /**
     * Display the template gallery for selecting a coach card template.
     */
    public function templates()
    {
        ...
        if ($user->type === 'company') {
            ...
            if ($companyUser && $companyUser->type === 'company') {
                $hasActivePlan = $companyUser->hasActiveSubscription();
                $currentPlan = $companyUser->plan;
            }
        } elseif ($user->type === 'superadmin') {
            $hasActivePlan = true; // Superadmins always have access
        }
        
        // Get plans for subscription modal
        $plans = [];
        $paymentMethods = [];
        ...
        if (!$hasActivePlan) {
            ...
            if ($paymentSettings['is_bank_enabled'] ?? false) {
                $paymentMethods[] = ['id' => 'bank', 'name' => 'Bank Transfer'];
            }
            if ($paymentSettings['is_manually_enabled'] ?? false) {
                $paymentMethods[] = ['id' => 'manually', 'name' => 'Manual Payment'];
            }
        }
        
        return Inertia::render('vcard-builder/templates', [
    ...
    public function create(Request $request)
    {
        ...
        // Check if user has an active plan (company users only)
        if ($user->type === 'company') {
            if (!$user->hasActiveSubscription()) {
                // Store selected template in session for after payment redirect
                if ($selectedTemplate) {
                    session([
                ...
                return redirect()->route('plans.index', ['onboarding' => 1])
                    ->with('info', __('Please subscribe to a plan to create your Coach Card.'));
            }
        ...
        elseif ($user->type !== 'superadmin' && $user->created_by) {
            ...
            $companyUser = User::find($user->created_by);
            if ($companyUser && $companyUser->type === 'company') {
                if (!$companyUser->hasActiveSubscription()) {
                    return redirect()->route('plans.index')
                        ->with('info', __('Your company needs an active plan. Please contact your administrator.'));
                }
