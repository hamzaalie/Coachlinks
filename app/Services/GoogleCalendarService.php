...
{
    private ?int $settingsUserId = null;

    public function __construct(?int $settingsUserId = null)
    {
        $this->settingsUserId = $this->resolveSettingsUserId($settingsUserId);
    }
    ...
    {
        $calendarId = $this->getSettingValue('googleCalendarId');
        $jsonPath = $this->getSettingValue('googleCalendarJsonPath');
    ...
    {
        $enabled = $this->getSettingValue('googleCalendarEnabled');
        $calendarId = $this->getSettingValue('googleCalendarId');
        $jsonPath = $this->getSettingValue('googleCalendarJsonPath');
    ...
    {
        if (!$this->settingsUserId) {
            return null;
        }

        return Setting::where('user_id', $this->settingsUserId)->where('key', $key)->value('value');
    }

    private function resolveSettingsUserId(?int $settingsUserId): ?int
    {
        if ($settingsUserId) {
            $owner = User::find($settingsUserId);
            if ($owner && !in_array($owner->type, ['superadmin', 'company'], true)) {
                return $owner->created_by ?: $owner->id;
            }
        ...
        if (in_array($authUser->type, ['superadmin', 'company'], true)) {
            return $authUser->id;
        }

        return $authUser->created_by ?: $authUser->id;
    }
}