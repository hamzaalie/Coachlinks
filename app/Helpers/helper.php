...
if (! function_exists('settings')) {
    function settings($user_id = null)
    {
        ...
        if (auth()->check() && auth()->user()->type !== 'superadmin') {
            ...
            if ($superAdmin) {
                $superAdminKeys = [
                    'decimalFormat', 'defaultCurrency', 'thousandsSeparator', 'floatNumber', 'currencySymbolSpace',
                    'currencySymbolPosition', 'dateFormat', 'timeFormat', 'calendarStartDay', 'defaultTimezone',
                ...
                // Enforce superadmin branding for non-superadmin users.
                foreach (['logoDark', 'logoLight', 'favicon', 'titleText', 'footerText'] as $brandKey) {
                    if (array_key_exists($brandKey, $superAdminSettings)) {
                        $userSettings[$brandKey] = $superAdminSettings[$brandKey];
                    }
                }
            }
        }
...
if (! function_exists('processPaymentSuccess')) {
    function processPaymentSuccess($data)
    {
        ...
        $wasActiveSubscription = $user->hasActiveSubscription();
        ...
        // Send billing emails for successful payments.
        $billingEmailService = app(\App\Services\BillingEmailService::class);
        $billingEmailService->sendPaymentReceipt($user, $planOrder);

        if ($wasActiveSubscription) {
            $billingEmailService->sendSubscriptionRenewalSuccess($user, $planOrder);
        } else {
            $billingEmailService->sendSubscriptionActivated($user, $planOrder);
        }
...
if (! function_exists('defaultSettings')) {
    ...
    function defaultSettings()
    {
        return [
            ...
            'titleText' => 'CoachLinks',
            'footerText' => '© 2025 CoachLinks. All rights reserved.',
            'themeColor' => 'green',
            'customColor' => '#10b981',
